    ...
    %POSITIONINT Position controller via single integrator dynamics
        ...
        % Error checking
            ...
            if(~isempty(norms(to_normalize)))                        
                dx(1, to_normalize) = velocity_magnitude_limit*dx(1, to_normalize)./norms(to_normalize);
                dx(2, to_normalize) = velocity_magnitude_limit*dx(2, to_normalize)./norms(to_normalize);
            end
