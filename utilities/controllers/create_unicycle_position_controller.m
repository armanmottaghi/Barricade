...
function [ created_position_controller ] = create_unicycle_position_controller(varargin)
    ...
    velocity_magnitude_limit = p.Results.VelocityMagnitudeLimit;
    ...
    % a unicycle-velocity-valued vector.
        ...
        for i = 1:N_poses
            ...
            dxu(1, i) = linear_velocity_gain*dist*cos(rot_error);
            dxu(2, i) = angular_velocity_gain*dist*sin(rot_error);
        end
        
        % Normalize velocities to magnitude
        norms = arrayfun(@(idxu) norm(dxu(:, idxu)), 1:N_poses);
        to_normalize = norms > velocity_magnitude_limit;
        if(~isempty(norms(to_normalize)))                        
            dxu(1, to_normalize) = velocity_magnitude_limit*dxu(1, to_normalize)./norms(to_normalize);
            dxu(2, to_normalize) = velocity_magnitude_limit*dxu(2, to_normalize)./norms(to_normalize);
        end
